...
> **Implementation status.** M1, M2, and the story group of M6 are
> implemented: `crates/aurum-mcp` ships a zero-dependency headless server with
> 31 `aurum_*` tools over stdio, driven by `aurum mcp`. See
> `crates/aurum-mcp/README.md`. The optional editor integration (M3 onward) is
> not started — it is the only part that requires the third-party toolkit.
